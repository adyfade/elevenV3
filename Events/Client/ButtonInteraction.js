    ...
    async execute(client, interaction) {
            ...
            if (button.permissions.voteRequired) {
                ...
                if (!voted && !client.owners.includes(interaction.member.user.id)) {
                    return interaction.reply({
                        embeds: [client.embed().setColor(client.color).setDescription(`You Need To [Vote](https://top.gg/bot/990494496817049690/vote) For Me To Use This Command!`),],
                        components: [client.row().addComponents(client.button().setStyle(client.config.button.link).setLabel("Vote").setURL("https://top.gg/bot/990494496817049690/vote"))],
                        ephemeral: true
                    });
                }
            }
        ...
        if (button.player) {
            if (button.player.voice) {
                if (!interaction.member.voice.channel) return await interaction.reply({ content: `> <:11:1288062188854050887> **You must be connected to a voice channel to use this \`${button.name}\` button.**`, ephemeral: true });
